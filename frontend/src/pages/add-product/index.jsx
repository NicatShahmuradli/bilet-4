...
import {deleteProductById, getAllProducts} from "../../services/api/product.js"
...
function AddProduct() {
  ...
  const columns = [
    {
      ...
      render:(productImg)=>(
        <img src={productImg} alt="Product Image" style={{maxWidth:"150px", maxHeight:"100px"}}/>
      )
    },
    {
      title: 'Name',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button
          ...
          onClick={async () => {
            try {
              const res = await deleteProductById(record._id);
  ...
  return (
    <>
      ...
      <Table rowKey={(x)=>x._id} columns={columns} dataSource={products} onChange={onChange} />
    </>
  );
}
